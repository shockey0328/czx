# `bi_pub_pub_fin_moneyrecord_revenue_1d`

- 层级：`bi`
- 本地表描述：产品事业部
- 主题标签：transaction_payment, finance
- 数据粒度：按 case when product_id in('xuekewang', 'ebeike') then '资源库' when product_id = 'zujuanwang' then '题库' when product_id = 'aiyanxiu' then 'AI研修' else '其他' end, user_id, dt 聚合
- 分区字段：dt
...
- 写入方式：overwrite
- 上游表：`dwd.dwd_pub_pub_fin_moneyrecord_di`
