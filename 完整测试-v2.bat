...
echo ========================================
echo 橙子学数据看板 - 完整测试工具 v2
echo ========================================
echo.
echo 此工具将测试所有看板的数据加载情况
echo （包括新增的月度核心数据静态版本）
echo.
echo ========================================
echo.
...
if exist "核心数据看板（周度）\data.js" (
    echo ✓ 核心数据看板（周度） data.js 存在
) else (
    echo ✗ 核心数据看板（周度） data.js 不存在
    set ALL_OK=0
)

if exist "核心数据看板（月度）\data.js" (
    echo ✓ 核心数据看板（月度） data.js 存在
) else (
    echo ✗ 核心数据看板（月度） data.js 不存在
    set ALL_OK=0
)
...
echo   - 查看文件大小和数据集信息
echo.
echo 【测试核心数据看板.html】（周度）
echo   - 测试核心数据的数据结构
echo   - 检查趋势图所需的字段
echo   - 验证日度数据格式
echo.
echo 【测试月度核心数据看板.html】（新增）
echo   - 测试月度核心数据结构
echo   - 验证静态HTML版本
echo   - 无需React服务器！
echo.
...
echo   3. 测试周度/月度切换
echo   4. 测试不同看板类型
echo   5. 月度核心数据现在是静态HTML，无需启动服务器！
echo.
echo 如果有测试失败：
